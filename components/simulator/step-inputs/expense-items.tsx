...
interface Props {
  ...
  onChange: (items: ExpenseItemInput[]) => void;
}

export function ExpenseItems({ items, onChange }: Props) {
  function addItem() {
  ...
  function updateItem(idx: number, field: keyof ExpenseItemInput, value: string | number) {
  ...
  return (
    <section className="mb-8">
      <div className="flex items-center gap-2 mb-4">
        <h3 className="font-semibold text-gray-800">Expense Items</h3>
        <span className="text-gray-400 text-sm">ⓘ</span>
      </div>
      <div className="space-y-3">
        {items.map((item, idx) => (
          <div key={item.id ?? idx} className="border border-gray-200 rounded-xl p-4 flex items-center gap-3">
